...
export function collectRefs(data) {
    ...
    return uniq(refs.reduce((acc, x) => acc.concat(x), []));
}
 ...
 */
export function transformSchema(schema, transformers = []) {
...
export async function compileSchema(schema, refs) {
    const ajv = new Ajv({
        strictTypes: false,
        allErrors: true,
        code: {
