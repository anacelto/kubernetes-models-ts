import { collectRefs, generateImports, generateInterface, getAPIVersion } from "@kubernetes-models/generate";
import { formatComment, trimPrefix, trimSuffix } from "@kubernetes-models/string-util";
import { mapValues, omit } from "lodash";
import { getClassName, getInterfaceName, getShortClassName, getShortInterfaceName, trimRefPrefix } from "../string.mjs";
import { getRelativePath, getSchemaPath, isAPIMachineryID } from "../utils.mjs";
...
export default function ({ getDefinitionPath, externalAPIMachinery }) {
    return async (definitions) => {
        return definitions.map((def) => {
            ...
            const imports = [];
            const gvk = def.gvk?.[0];
            const typing = generateInterface(gvk ? omitTypeMetaDescription(def.schema) : def.schema, {
                getRefType,
            ...
            for (const ref of refs) {
                ...
                if (externalAPIMachinery && isAPIMachineryID(ref)) {
                    imports.push({
                        name,
                        path: `@kubernetes-models/apimachinery/${trimPrefix(ref, "io.k8s.apimachinery.pkg.")
                            .split(".")
                            .join("/")}`
                    });
                }
...
static apiVersion: ${shortInterfaceName}["apiVersion"] = "${getAPIVersion(gvk)}";
