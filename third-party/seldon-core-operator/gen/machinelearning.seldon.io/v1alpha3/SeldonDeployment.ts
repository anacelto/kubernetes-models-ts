...
import { validate } from "../../_schemas/MachinelearningSeldonIoV1alpha3SeldonDeployment";

/**
 * SeldonDeployment is the Schema for the seldondeployments API
 */
export interface ISeldonDeployment {
  /**
 ...
 * SeldonDeployment is the Schema for the seldondeployments API
 */
export class SeldonDeployment extends Model<ISeldonDeployment> implements ISeldonDeployment {
  "apiVersion": ISeldonDeployment["apiVersion"];
  "kind": ISeldonDeployment["kind"];
  "metadata"?: ISeldonDeployment["metadata"];
  "spec"?: ISeldonDeployment["spec"];
  "status"?: ISeldonDeployment["status"];

static apiVersion: ISeldonDeployment["apiVersion"] = "machinelearning.seldon.io/v1alpha3";
static kind: ISeldonDeployment["kind"] = "SeldonDeployment";
static is = createTypeMetaGuard<ISeldonDeployment>(SeldonDeployment);

constructor(data?: ModelData<ISeldonDeployment>) {
  super();

  this.setDefinedProps({
    apiVersion: SeldonDeployment.apiVersion,
    kind: SeldonDeployment.kind,
    ...data
  } as ISeldonDeployment);
}
}


setValidateFunc(SeldonDeployment, validate as ValidateFunc<ISeldonDeployment>);
