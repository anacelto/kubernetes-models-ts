...
import { validate } from "../../_schemas/EventingKnativeDevV1Broker";
 ...
 * Broker collects a pool of events that are consumable using Triggers. Brokers provide a well-known endpoint for event delivery that senders can use with minimal knowledge of the event routing strategy. Subscribers use Triggers to request delivery of events from a Broker's pool to a specific URL or Addressable endpoint.
 */
export class Broker extends Model<IBroker> implements IBroker {
  "spec"?: IBroker["spec"];
...
static apiVersion: IBroker["apiVersion"] = "eventing.knative.dev/v1";
static kind: IBroker["kind"] = "Broker";
static is = createTypeMetaGuard<IBroker>(Broker);
...
setValidateFunc(Broker, validate as ValidateFunc<IBroker>);
