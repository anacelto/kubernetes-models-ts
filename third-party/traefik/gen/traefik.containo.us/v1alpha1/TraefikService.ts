...
import { validate } from "../../_schemas/TraefikContainoUsV1alpha1TraefikService";
 ...
 * TraefikService is the CRD implementation of a Traefik Service. TraefikService object allows to: - Apply weight to Services on load-balancing - Mirror traffic on services More info: https://doc.traefik.io/traefik/v2.10/routing/providers/kubernetes-crd/#kind-traefikservice
 */
export class TraefikService extends Model<ITraefikService> implements ITraefikService {
  "apiVersion": ITraefikService["apiVersion"];
  "kind": ITraefikService["kind"];
  "metadata": ITraefikService["metadata"];
  "spec": ITraefikService["spec"];

static apiVersion: ITraefikService["apiVersion"] = "traefik.containo.us/v1alpha1";
static kind: ITraefikService["kind"] = "TraefikService";
static is = createTypeMetaGuard<ITraefikService>(TraefikService);

constructor(data?: ModelData<ITraefikService>) {
  super();

  this.setDefinedProps({
    apiVersion: TraefikService.apiVersion,
    kind: TraefikService.kind,
    ...data
  } as ITraefikService);
}
}
...
setValidateFunc(TraefikService, validate as ValidateFunc<ITraefikService>);
